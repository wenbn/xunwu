...
public class AdminController extends BaseController {
     ...
     * @param username
     * @param password
     * @param roleType 登录角色  0 外获  1 外看  2 区域长
     * @return
     */
    @RequestMapping(value = "login" ,method = RequestMethod.POST)
    @ResponseBody
    public String login(@RequestParam("username") String username,@RequestParam("password") String password ,@RequestParam("roleType") String roleType){
        ResultVo vo = new ResultVo();
        Subject subject = SecurityUtils.getSubject();
        //Session session = subject.getSession();
        if(!StringUtil.hasText(roleType)){
            roleType = "0";
        }
        ...
        try {
            ...
            if(!username.equals("15012940205")){
                Boolean isContinue = false;
                if(roleType.equals("0") && roleName.equals("外获业务员")){
                    isContinue = true;
                }else if(roleType.equals("1") && roleName.equals("外看业务员")){
                    isContinue = true;
                }else if(roleType.equals("2") && roleName.equals("钥匙管理员")){
                    isContinue = true;
...
//        vo.setDataSet("当前在线用户数量："+count);
    ...
    public String validateSmsCode(HttpServletRequest request){
        ...
        try {
            ...
            if(!StringUtil.hasText(smsCode) || !StringUtil.hasText(mobile)){
    ...
    public String forgetPassword(HttpServletRequest request,HttpServletResponse response){
        ...
        try {
            ...
            String mobile = request.getParameter("mobile");//新密码
            if(!StringUtil.hasText(password) || !StringUtil.hasText(mobile)) {
                result.setResult(ResultConstant.SYS_REQUIRED_TIMEOUT_ERROR);
                result.setMessage(ResultConstant.SYS_REQUIRED_TIMEOUT_ERROR_VALUE);
                return JsonUtil.toJson(result);
            }
