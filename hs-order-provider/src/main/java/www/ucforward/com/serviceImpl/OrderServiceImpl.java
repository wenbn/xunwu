...
public class OrderServiceImpl<T> implements OrderService<T> {
    ...
    @Autowired
    private HsHousingOrderRefundLogDao hsHousingOrderRefundLogDao;
    ...
    public ResultVo insert(Object o) throws Exception {
        ...
        try{
            ...
            }else if (o instanceof HsHousingOrderRefundLog) {
                result = hsHousingOrderRefundLogDao.insert((HsHousingOrderRefundLog) o);
            }
            if(result>0){//操作成功
    ...
    public ResultVo selectCustomColumnNamesList(T t, Map<Object,Object> condition) throws Exception {
        ...
        try{
            ...
            }else if (t.hashCode() ==  HsHousingOrderLog.class.hashCode()) {
                result = hsHousingOrderLogDao.selectCustomColumnNamesList(condition);
            }
            vo.setResult(ResultConstant.SYS_REQUIRED_SUCCESS);
            vo.setMessage(ResultConstant.SYS_REQUIRED_SUCCESS_VALUE);
            vo.setDataSet(result.get("data"));
    ...
    public ResultVo batchUpdate(Object o,Map<Object,Object> condition) throws Exception {
        ...
        try{
            if (o instanceof HsSystemOrderPool) {//
                ...
                List<HsHousingOrder> data = (List<HsHousingOrder>) condition.get("data");
                result = hsHousingOrderDao.batchUpdate(data);
                if(result>0){
                    List<HsHousingOrderLog> logData = (List<HsHousingOrderLog>) condition.get("logData");
                    if(logData!=null && logData.size() >0){
                        hsHousingOrderLogDao.batchInsert(logData);
                    }
                }
            }
            if(result>0){//操作成功
    ...
    @Override
    public ResultVo updateByCondition(Object o,Map<Object, Object> condition){
        ResultVo vo = new ResultVo();
        int result = -1;//
        try{
            if (o instanceof HsHousingOrder) {
                result = hsHousingOrderDao.updateByCondition(condition);
            }
            if(result>0){//操作成功
                vo.setResult(ResultConstant.SYS_REQUIRED_SUCCESS);
                vo.setMessage(ResultConstant.SYS_REQUIRED_SUCCESS_VALUE);
