...
public class CustomController {
    ...
    public String houseEdit(HttpServletRequest request,HsMainHouse hsMainHouse, HsHouseCredentialsData hsHouseCredentialsData,HsHouseImg hsHouseImg){
        ...
        try {
            ...
            String reraPermitNo = StringUtil.trim(request.getParameter("reraPermitNo"));
            if(houseId == -1 || !StringUtil.hasText(reraPermitNo)){
                //房源Id是否存在
            ...
            mainHouse.setUpdateBy(userid);
            mainHouse.setUpdateTime(new Date());
            result = housesManager.houseEdit(mainHouse,houseCredentialsData,houseImg,condition);
        } catch (Exception e) {
            e.printStackTrace();
            result.setResult(ResultConstant.SYS_REQUIRED_TIMEOUT_ERROR);
            result.setMessage(ResultConstant.SYS_REQUIRED_TIMEOUT_ERROR_VALUE);
    ...
    public String updateFeedback(HttpServletRequest request, @Valid HsFeedback hsFeedback, BindingResult br){
        ...
        try {
            ...
            ActiveUser user = (ActiveUser) SecurityUtils.getSubject().getPrincipal();
            Integer userid = user.getUserid();
            int status = StringUtil.getAsInt(StringUtil.trim(request.getParameter("status")));
            hsFeedback.setUpdateBy(userid);
            //处理完成 记录关单人
            if(status == 1){
                hsFeedback.setCloseBy(userid);
            }
    ...
    public String getObtainedList(HttpServletRequest request){
        ...
        //下架编号
        String keyword = StringUtil.trim(request.getParameter("obtainedCode"));

        Integer status = StringUtil.getAsInt(StringUtil.trim(request.getParameter("status")));
        if(status == 1){
            return JsonUtil.toJson(ResultVo.error(ResultConstant.SYS_REQUIRED_PARAMETER_ERROR, ResultConstant.SYS_REQUIRED_PARAMETER_ERROR_VALUE));
        }
        if(status > -1){
    ...
    public String updateObtained(HttpServletRequest request, @Valid HsHouseObtained hsHouseObtained, BindingResult br){
        ...
        hsHouseObtained.setUpdateTime(new Date());
        Integer status = hsHouseObtained.getStatus();
        if(status == 2){
            //下架来源 (1外部 2外获 3外看 4区域长 5PC)
            hsHouseObtained.setPlatform(5);
        }
     ...
     * @return
     */
    @PostMapping(value="/house/apply/withdraw")
    public String houseApplyWithdraw(HttpServletRequest request, HsHouseObtained houseObtained) {
        ResultVo result = new ResultVo();
        try {
            ...
            houseObtained.setCreateBy(userId);
            houseObtained.setCreateTime(new Date());
            //下架来源 (1外部 2外获 3外看 4区域长 5PC)
            houseObtained.setPlatform(5);
            result = housesManager.houseApplyWithdraw(houseObtained);
        } catch (Exception e) {
