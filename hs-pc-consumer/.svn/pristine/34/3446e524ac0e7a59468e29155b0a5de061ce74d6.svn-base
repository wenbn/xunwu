...
public class PersonalCenterController extends BaseController {
     ...
     * @throws Exception
     */
    @PostMapping(value="center/user/logo/update")
    public String updateUserInfo(String userLogo) throws Exception {
        ...
        ActiveUser user = (ActiveUser) SecurityUtils.getSubject().getPrincipal();
        HsSysUser updateUser = new HsSysUser();
        updateUser.setId(user.getUserid());
        updateUser.setUserLogo(userLogo);
        ResultVo vo = memberManager.updateUser(updateUser,null);
        return JsonUtil.toJson(vo);
    }
    ...
    public String getUserCloseOrderRecordList(HttpServletRequest request) throws Exception {
        ...
        //0 投诉订单 1 反馈
        int type = StringUtil.getAsInt(StringUtil.trim(request.getParameter("type")));
        if(type < 0){
            vo = ResultVo.error(ResultConstant.SYS_REQUIRED_PARAMETER_ERROR,ResultConstant.SYS_REQUIRED_PARAMETER_ERROR_VALUE);
            return JsonUtil.toJson(vo);
        }
        ...
        if(type == 0){
            ...
            vo = housesManager.getFeedbackList(condition);
        }
        return JsonUtil.toJson(vo);
    }
