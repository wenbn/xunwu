...
public class BoothController extends BaseController{
    ...
    public String getBoothList(HttpServletRequest request) throws Exception {
        ...
        int boothState = StringUtil.getAsInt(request.getParameter("boothState"),-1); //展位状态（0：启用，1：禁用）
        int isDel = StringUtil.getAsInt(request.getParameter("isDel"),-1); //是否删除0:未删除，1：删除
        int boothExclusive = StringUtil.getAsInt(request.getParameter("boothExclusive"),-1); //该展位的专属(1:PC, 2:TC, 3:APP)
        ...
        if(boothState != -1){
            condition.put("boothState",boothState);
        }
        if(isDel != -1){
            condition.put("isDel",isDel);
        }
        if(boothExclusive != -1){
            condition.put("boothExclusive",boothExclusive);
        }
