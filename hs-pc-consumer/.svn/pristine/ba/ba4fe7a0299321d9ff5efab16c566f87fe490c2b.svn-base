...
//    @RequiresPermissions("role:list")
    ...
    public String getRolelList(HttpServletRequest request) throws Exception {
        ResultVo vo = null;
        int pageIndex = StringUtil.getAsInt(request.getParameter("pageIndex")); //当前页
        int pageSize = StringUtil.getAsInt(request.getParameter("pageSize")); //每页显示多少数据
        ...
        if(isDel != -1){
            condition.put("isDel",isDel); //页显示条数
        }
