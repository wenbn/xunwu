...
//    @RequiresPermissions("article:list")
    ...
    public String getArticleList (HttpServletRequest request) throws Exception {
        ...
        int status = StringUtil.getAsInt(request.getParameter("status")); //状态  0 未启用  1 启用
        int articleType = StringUtil.getAsInt(request.getParameter("articleType")); //状态  0 未启用  1 启用
        ...
        if(status != -1 ){
            condition.put("status", status); //页显示条数
        }
        if(articleType != -1 ){
            condition.put("articleType", articleType); //页显示条数
        }
