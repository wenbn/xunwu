...
public class AdvertController extends BaseController {
    ...
    public String updateAdvert(HsAdvertData advertData) throws Exception {
        ...
        if (adType == 1) {//广告类型(1:图片类型, 2:文字类型)
            if (!StringUtil.hasText(adImgUrl)) {
                return JsonUtil.toJson(ResultVo.error(ResultConstant.SYS_REQUIRED_PARAMETER_ERROR, "请上传广告图片"));
            }
        } else if (adType == 2) {
            if (!StringUtil.hasText(adContent)) {
                return JsonUtil.toJson(ResultVo.error(ResultConstant.SYS_REQUIRED_PARAMETER_ERROR, "请输入广告文字内容"));
            }
        }
     ...
     * @return
     */
    @RequiresPermissions("advert:detail")
