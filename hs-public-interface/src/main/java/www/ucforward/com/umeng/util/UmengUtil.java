...
public class UmengUtil {
    ...
    private static final String REGIONALCHIEF_ANDROID_APPKEY = PropertiesUtils.loadProps("/umeng/umeng.properties").getProperty("regionalChief_android_appkey");
    private static final String REGIONALCHIEF_ANDROID_MESSAGE_SECRET = PropertiesUtils.loadProps("/umeng/umeng.properties").getProperty("regionalChief_android_messageSecret");
    private static final String REGIONALCHIEF_IOS_APPKEY = PropertiesUtils.loadProps("/umeng/umeng.properties").getProperty("regionalChief_ios_appkey");
    private static final String REGIONALCHIEF_IOS_MESSAGE_SECRET = PropertiesUtils.loadProps("/umeng/umeng.properties").getProperty("regionalChief_ios_messageSecret");
     ...
     * @param msg      推送内容
     * @param platform 平台类型 测试:0 外部:1 外获:2 外看:3 区域长:4
    ...
    public static boolean aliasSend(Integer language,String alias, String msg, Integer platform, Integer msgCode) throws Exception {
        ...
        //别名类型 必须与别名对应 测试test 外部:external 外获:outOf 外看:outLook 区域长:regionalChief
        String aliasType;
        //判断用户是否开启消息推送
        ...
        switch (platform) {
            ...
            case 3:
                ...
                break;
            case 4:
                androidAppKey = REGIONALCHIEF_ANDROID_APPKEY;
                androidMessageSecret = REGIONALCHIEF_ANDROID_MESSAGE_SECRET;
                iosAppKey = REGIONALCHIEF_IOS_APPKEY;
                iosMessageSecret = REGIONALCHIEF_IOS_MESSAGE_SECRET;
                aliasType = spliceAliasType(aliasSplit.length, "regionalChief");
                break;
            default:
...
//        String title = getTitle(language, platform, msgCode);
        ...
        /**
         * 系统消息为群发消息，后台不能根据用户的设置去判断是否推送。交由前端判断是否接受
         * title为"sysMsg"表示为系统消息，此时前端需要去获取用户的设置信息，判断是否接受这个推送
         */
        String title = "sysMsg";
        String androidAppKey;
        String androidMessageSecret;
...
//        iosBroadcast.setTestMode();
    ...
    private static String getTitle(Integer language,Integer platform,Integer msgCode){
        ...
        Optional<Map<Object, Object>> codeOptional = msgList.stream().filter(map -> map.get("code").equals(msgCode)).findFirst();
        Map<Object, Object> code = new HashMap<>(16);
