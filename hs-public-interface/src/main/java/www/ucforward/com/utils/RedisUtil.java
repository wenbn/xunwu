...
public class RedisUtil {
        ...
        if(pool == null){
            synchronized (RedisUtil.class){
                if(pool == null){
                    ...
                    JedisShardInfo jedisShardInfo = new JedisShardInfo(properties.getProperty("redis.host"), properties.getProperty("redis.port"));
                    jedisShardInfo.setPassword(properties.getProperty("redis.pwd"));
     ...
     * @return
     */
    public static synchronized <T> T safeGet(String key) {
        ShardedJedis jedis = null;
        T value = null;
        try {
            jedis = getJedis();
     ...
     * @param object
     */
    public static Boolean safeSet(String key, Object object) {
        return safeSet(key,object,60*30);
    }
     ...
     * @param time 过期时间
     */
    public static synchronized Boolean safeSet(String key, Object object, int time) {
        ShardedJedis jedis = null;
        try {
            jedis = getJedis();
            if(jedis != null) {
     ...
     * @param key
     */
    public static synchronized Boolean safeDel(String key){
        ShardedJedis jedis = null;
        try {
            jedis = getJedis();
            if (jedis != null) {
    ...
    public static synchronized Boolean existKey(final String key) {
        ShardedJedis jedis = null;
        try {
            jedis = getJedis();
...
//        jedis.close();
//        System.out.println(jedis.get("test"));
//        ShardedJedis jedis10 = pool.getResource();
...
////
//        ShardedJedis jedis = RedisUtil.getJedis();
//        RedisUtil.safeSet("user", SerializationUtil.serialize(map));
//        byte[] bs = RedisUtil.safeGet("user");
//        Map desUser = (Map) SerializationUtil.deserialize(bs);
//        System.out.println(desUser);
//
//        RedisUtil.safeSet("user",map);
//        Map<Object,Object> returnMap = RedisUtil.safeGet("user");
...
//        System.out.println("=="+sb.toString());
        ...
        String o= RedisUtil.safeGet("SYS_USER_REGISTER_IMG_CODE_KEY8618665956590121.35.100.81");
